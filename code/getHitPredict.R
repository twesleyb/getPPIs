...
getHitPredict <- function(organism="HitPredict", downloads=getwd()) {
	...
	# Progress Report.
	n <- dim(data)[1]
	message(paste(n,"PPIs compiled from the HitPredict database and mapped to Entrez IDs."))
	...
	message(paste0(n_missing, " (", percent_missing, ") ", 
		      "interactions are incompletely mapped to Entrez IDs and will be discarded."))
	# Remove unmapped ids.
